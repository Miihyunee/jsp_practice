...
<script>
	$(function() {
		...
		$("btnAllDel").click(function() {
			let arr = $("input[name=idx]"); // 체크박스 배열
			...
			document.from1.action = "/jsp03/memo_servlet/delete_all.do";
			...
			<form method="post" name="form1">
				<table class="table table-hover">
					<tr>
						...
						<th>메모</th>
						<th>날짜</th>
						<th><input type="button" value="선택삭제" id="btnAllDel"
							class="btn btn-danger"></th>
					</tr>

					<c:forEach var="row" items="${list}">
						<tr>
							<td><input type="checkbox" name="idx" vlaue="${row.IDX}"></td>
							<td>${row.IDX}</td>
							<td>${row.WRITER}</td>
							<td><a href="/jsp03/memo_servlet/view.do?idx=${row.IDX}">${row.MEMO}</a></td>
							<td>${row.POST_DATE}</td>
							<td><input type="button" value="삭제"
								onclick="memo_del('${row.IDX}')" class="btn btn-success"></td>
						</tr>
					</c:forEach>
				</table>
			</form>
