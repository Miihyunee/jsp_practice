		...
		<table border="1" width="400px">
			...
			<c:forEach var="row" items="${list}">
				<tr>
					...
					<td><input type="number" style="width: 50px" min="0" max="100"
						name="amount" value="${row.amount}"> <input type="hidden"
						name="product_code" value="${row.product_code}"> <input
						type="hidden" name="cart_id" value="${row.cart_id}"></td>
					<td><fmt:formatNumber value="${row.money}" pattern="#,###" /></td>
					<td><a
						href="/jsp03/cart_servlet/delete.do?cart_id=${row.cart_id}">삭제</a>
				</tr>
			</c:forEach>
			<tr>
				<td colspan="5" align="center"><c:if
						test="${map.sum_money > 0}">
				장바구니 금액 합계 :
				<fmt:formatNumber value="${map.sum_money}" pattern="#,###,###" />
						<br>
				배송료 : ${map.fee}<br>
